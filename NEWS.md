...
* **`hw3_export()` crashed on event files containing `category = 0`.**
  ...
  seasonal threshold has collapsed to a single constant, `seas == thresh`)
  produced a shorter label vector than the data.frame it was assigned into,
  throwing `replacement has fewer rows than data`. Fixed by offsetting the
  lookup so out-of-range codes map to `NA_character_` instead of being
  dropped, which now holds row count regardless of what category codes
  appear in the file.
