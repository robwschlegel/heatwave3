...
#'
#' @examples
detect3.df <- function(file_in, file_out = NULL, return_type = NULL){
  ...
  if(!is.null(return_type)){
    ...
    if(return_type == "df"){
      ...
      nc_csv$index_start <- as.Date(nc_csv$index_start, origin = min(terra::time(nc_rast))-1)
      nc_csv$index_peak <- as.Date(nc_csv$index_peak, origin = min(terra::time(nc_rast))-1)
      nc_csv$index_end <- as.Date(nc_csv$index_end, origin = min(terra::time(nc_rast))-1)
