...
Date fields (`date_start`, `date_peak`, `date_end`) are stored as
integer offsets in units of `"days since YYYY-MM-DD"`, where the
...
In addition to NetCDF, heatwave3 can export output as CSV, RDA, or
Parquet using `hw3_export()` or the `save_format` argument:
