...
grid, ca. 50,000 ocean pixels, 14,276 daily time steps), Apple M3 Pro:
...
-   **All-in-one pipeline** (`detect3`). Climatology, detection, and
    optional categories in a single call, with `return_df = TRUE` for
    interactive use.
...
-   **Automatic dimension detection**. Identifies lon/lat/time/SST from
    CF attributes, standard names, and units (works with GHRSST, OISST,
    OSTIA, ERA5, CMIP6, NEMO).
-   **Progress reporting**. Percentage-complete updates during long
    climatology and detection runs.
-   **Multiple output formats**. NetCDF always, plus optional CSV, RDA,
    or Parquet companion files.
...
`parallel::mclapply()` (fork-safe via `pthread_atfork`).
...
-   **Getting started**. Full pipeline walkthrough with spatial blob
    figures.
-   **NetCDF output internals**. Output file structure, CF compliance,
    and reading in R/Python/CDO.
