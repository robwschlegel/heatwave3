...
lat, ~49,800 ocean pixels, 14,276 daily time steps, 1982–2021).
...
# ~49,796 ocean pixels (remainder is land)
```
...
|--------|------------|-----------|-------|---------|
| heatwaveR (serial) | — | — | ~4,116 sec (68.7 min) | 1× |
...
2. **OpenMP parallelism** — each pixel is independent; OpenMP distributes
...
- The 12-thread run is 2.2× faster than single-threaded, not 12×, because the
  full 400×200 grid must first be read into memory (~900 MB for this dataset)
...
This benchmark demonstrates heatwave3 on a production-scale analysis: the
...
# Climatology from ~16,000 daily files (WMO 1991-2020 baseline)
t_clim <- system.time(
  ts2clm3(
    file_in  = ostia_dir,
    file_out = clim_file,
...
For comparison, processing 93,600 pixels serially with heatwaveR at ~0.083
sec/pixel would take approximately **2.2 hours**. heatwave3 completes the same
analysis in 3.4 minutes — a **~38× speedup**.
