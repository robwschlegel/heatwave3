...
1. `ts2clm3()` — compute seasonal and threshold climatologies
2. `detect_event3()` — detect events based on the climatologies
...
- **A single multi-timestep NetCDF file** (e.g. a merged time series)
...
Here we demonstrate the full pipeline using the bundled OSTIA test dataset (3×2
...
They demonstrate heatwave blob footprints, daily area evolution, centroid
...
# Detect spatial blobs — include voxel data for footprint maps
...
heatwave3 achieves significant speedups over per-pixel processing with
heatwaveR. On a 20×20 grid (400 pixels, 14,276 daily time steps each):

| Method | Time | Speedup |
|--------|------|---------|
| heatwaveR (serial, 400 pixels) | ~38 sec | 1× |
| heatwave3 (4 threads) | 0.53 sec | **~71×** |
