...
1. **Climatology file** (from `ts2clm3()`) — seasonal and threshold
   climatologies per pixel per day-of-year
2. **Event file** (from `detect_event3()`) — detected events stored as a CF
   ragged array
...
| `doy`    | (doy)     | — | Day of year index |
| `seas`   | (lon, lat, doy) | degC | Seasonal mean climatology |
| `thresh` | (lon, lat, doy) | degC | Threshold climatology (e.g. 90th percentile) |
...
Produced by `detect_event3()`. Uses a **flat event dimension** — each row
...
|----------|-------|-------------|
| `event_no` | — | Event number within the pixel |
...
- The event file uses a flat dimension rather than CF indexed ragged arrays
  ...
  pragmatic choice — flat layout is simpler to read and write, and all major
  tools handle it correctly.
